...
// Exact replica of gpu.cu's make_config() constant computation
// All intermediates computed in double precision, then cast to float,
// matching gpu.cu's OctaveConfig values bit-for-bit.
...
// gpu.cu loop: i=0 amp=1.0, then if*=2 vf*=0.5; i=1 amp=1.0
...
__device__ __forceinline__ uint32_t warp_reduce_add(uint32_t v) {
    #pragma unroll
    for(int o=16;o>0;o/=2) v+=__shfl_xor_sync(0xFFFFFFFF,v,o);
    return v;
}
__device__ __forceinline__ void compute_cell(const ImprovedNoise&n,int32_t ix,int32_t iy,int32_t iz,
...
__device__ __forceinline__ float interp(const GradDotTable&t,float fx_,float fy_,float fz_,float fx,float fy,float fz,
    uint8_t c000,uint8_t c100,uint8_t c010,uint8_t c110,uint8_t c001,uint8_t c101,uint8_t c011,uint8_t c111){
    ...
    KernelSeed1::Result *results, GradDotTable gdt)
{
    ...
    for(uint32_t idx=wg; idx<*inputs.len; idx+=nw) {
        ...
        int32_t cur_ix0=0,cur_ix1=0;bool have0=false,have1=false;
